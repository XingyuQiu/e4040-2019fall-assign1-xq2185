...
class TwoLayerNet(object):
    ...
    def loss(self, X, y):
        ...
        #            START OF YOUR CODE          #
        ###################################################  
        forward_ly1 = self.layer1.feedforward(X)
        forward_ly2 = self.layer2.feedforward(forward_ly1)
        loss,dout = softmax_loss(forward_ly2,y)
        
        ###################################################
        # TODO: Backpropogation,here is just one dense   #
        # layer, it should be pretty easy            #
        ###################################################      
        back_ly1 = self.layer2.backward(dout)
        back_ly0 = self.layer1.backward(back_ly1)
    ...
    def predict(self, X):
        ...
        #            START OF YOUR CODE          #
        ###################################################  
        # Use the first dense layer
        out_ly1 = self.layer1.feedforward(X)
        out_ly2 = self.layer2.feedforward(out_ly1)
        predictions = np.argmax(out_ly2, axis = 1)
